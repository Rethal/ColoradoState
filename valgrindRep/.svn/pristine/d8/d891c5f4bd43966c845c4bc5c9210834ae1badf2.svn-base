...
int stick::comparePosesMulti(const stick& StickShort, const stick& StickLong) {
	...
	for (unsigned int i = 0; i < StickShort.timeVector.size(); i++) {
		...
		for (unsigned int k = 0; k < StickLong.timeVector.size(); k++) {
			...
			double poseCompare = poseVectorCompare(poseVectorShort, poseVectorLong);
			poseDistances.push_back(poseCompare);
		}
...
int stick::dynamicMatch(const stick& firstIn, const stick& secondIn) {
	//passes short in first
	if (firstIn.timeVector.size() > secondIn.timeVector.size()) {
		if (comparePosesMulti(secondIn, firstIn) == -1) {
			return -1;
