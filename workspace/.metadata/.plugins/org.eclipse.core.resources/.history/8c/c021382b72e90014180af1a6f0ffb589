public class Tree {
	...
	public Integer postorderEval(String indent, BST symTab) throws BSTException{
		Integer res = null;
		if(!isEmpty()){
	...
	public Integer postorderEval(TreeNode node, String indent, BST symTab) throws BSTException{
		...
		Symbol sym = symTab.retrieveItem(indent);
//		int val = sym.getVal();
		if(rootItem.equals("+")||rootItem.equals("-")||rootItem.equals("*")||rootItem.equals("/")){
			int left = postorderEval(root.getLeft(),indent,symTab);
			int right = postorderEval(root.getRight(),indent,symTab);
			if (rootItem.equals("+")){
				return left+right;
			}
			if (rootItem.equals("-")){
		...
		else return Integer.valueOf(rootItem);
	}
