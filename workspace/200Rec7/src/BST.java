...
public class BST {
	...
	public String deleteItem(BSTNode node, String key){
		String treeItem;
		if(node==null){
			treeItem=null;
		}
			...
			if(key.compareTo(nodeItem) == 0){
				//found
				//node.setItem(node.delete(node));
				node.setLeft(node.delete(node.delete(node)));
				treeItem = nodeItem;
			}
			else if(key.compareTo(nodeItem) < 0)
				//search left
				treeItem = deleteItem(node.getLeft(), key);
			else
				// search right
				treeItem = deleteItem(node.getRight(), key);
			//4.setItem(delete("4"))
			//8.setLeft(delete("6")); returns "7"
		}
		return treeItem;
	}
