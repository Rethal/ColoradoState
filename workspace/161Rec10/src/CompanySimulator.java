...
public class CompanySimulator {
	...
	private void initCompany() {
		...
		int managerID = 1;
		for (int i = 0; i < 3; i++) {
			employees.add(new Manager(names[id], id, managerID));  // we assume that all managers have a common boss that is not referenced.
			id += 1;
			for (int j = 0; j < 2; j++) {
	...
	private void manageCrisis(Employee emp) {
		...
		int man = emp.getManagerID();
		for (Employee boss : employees) {
			if(boss instanceof Manager){
				Manager m = (Manager)boss;
				if (m.getManagerID()==man){
					m.handleCrisis();
					System.out.println(m.getName() + ": I've got it!");
				}
			}
		}
	}
