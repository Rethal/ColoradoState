...
public class Assign4 {
	...
	public long groups (int n, int k){
		...
		else return groups(n-1,k)+groups(n-1,k-1)+groups(n-1,k);
	}
	...
	public int mkChange(int n){
		...
		if(n>=1&&n<5){
			...
			return mkChange(n-10)+mkChange(n-5)*mkChange(n-15)+mkChange(n-15)+mkChange(n-20);
		}
		else if(n>=25){
			return mkChange(n-5)+mkChange(n-10)+mkChange(n-25);
		}
		else return 0;
		 */
		int ways = 0;
		for(int i=3;i>=0;i--){
			if(i>0){
				for(int j=coins[i];j<=n;j+=coins[i]){
				ways++;
			}
			}
			if(i-1>0){
				for(int j=coins[i];j<=n;j+=coins[i-1]){
					ways++;
				}
			}
			if(i-2>0){
				for(int j=coins[i];j<=n;j+=coins[i-2]){
					ways++;
				}
			}
			if(i==0){
