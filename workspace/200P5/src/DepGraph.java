...
public class DepGraph {	
	...
	private boolean graphContains(String source){
		//return graph.contains(new AdjList(source));
		for(int i=0;i<graph.size();i++){
	...
	public void addAdjList(String source) throws GraphException{
		if (graphContains(source))
			...
			AdjList list = new AdjList(source);
			graph.add(list);
			n++;
		}
	}
	...
	private AdjList getAdjList(String source) throws GraphException{
		...
		return null;
			//graph.get(graph.indexOf(source));
	}
	...
	public void addDest(String source, String dest) throws GraphException{
		if(!graphContains(source)) throw new GraphException("graph does not contain source: "+ source);
		if(!graphContains(dest)) throw new GraphException("graph does not contain dest: "+ dest);
