...
public class Assign3 {
	...
	public ArrayList<String> intersection( ArrayList<String> AL1, ArrayList<String> AL2){
		...
		if(!AL1.isEmpty()&&!AL2.isEmpty()&&!AL1.equals(AL2)){
			return match(AL1,AL2,empty,AL1.size()-1,0);
		}
		else return empty;
	}
	private ArrayList<String> match(ArrayList<String> AL1, ArrayList<String> AL2, ArrayList<String> AL3, int size, int location){
		check(0,AL2.size()-1,AL1.get(location),AL2,AL3);
		if(location==size){
			return AL3;
		}
		else return match(AL1,AL2,AL3,size,location+1);
	}

	private void check(int location, int size, String letter, ArrayList<String> AL2, ArrayList<String>AL3){
		if(location<size){
			if(AL2.get(location)==letter){
				AL3.add(letter);
			}
			check(location+1,size,letter,AL2,AL3);
		}
	}
