...
int main(int argc, char *argv[]) {
	...
	if (transform != "sort" && transform != "rotate" && transform != "swap" && transform != "left" && transform != "transform") {
		cerr << "error: please use an appropriate transformation" << endl;
		return -1;
	}

	stick Stickman(inFile, transform, num, outFile);
	Stickman.changePoses(transform, num);
	Stickman.posesPrint(outFile);
	//PA9
