...
ofstream audi;
int FRAMED;
bool bySpecificPoint(vector<Point3D>& A, vector<Point3D>& B) {
	if (FRAMED % 3 == 0) {
		FRAMED = FRAMED / 3;
		return(A[FRAMED].X() < B[FRAMED].X());
	}
	else if (FRAMED % 3 == 1) {
		FRAMED = FRAMED / 3;
		return(A[FRAMED].Y() < B[FRAMED].Y());
	}
	else {
		FRAMED = FRAMED / 3;
		return(A[FRAMED].Z() < B[FRAMED].Z());
	}
	return true;
}
...
int stick::changePoses(string transform, double num) {
	if (transform == "sort") {
		FRAMED = (int)num;
		sort(timeVector.begin(), timeVector.end(), bySpecificPoint);
	return 0;
	}
	else if (transform == "rotate") {
		num = (int)num;
		if (num < 1 || num>75) {
			cerr << "error: fourth parameter must be between 1 and 75" << endl;
				return -1;
		}

		for (unsigned int i = 0; i < timeVector.size(); i++) {
			vector<double> totalPose;
			for (unsigned int j = 0; j < timeVector[i].size(); j++) {
				totalPose.push_back(timeVector[i][j].X());
				totalPose.push_back(timeVector[i][j].Y());
				totalPose.push_back(timeVector[i][j].Z());
			}
			rotate(totalPose.begin(), totalPose.begin() + num, totalPose.end());
			vector<Point3D> newPose;
			for (unsigned int j = 0; j < totalPose.size(); j+=3) {
					newPose.push_back(Point3D(totalPose[j], totalPose[j+1], totalPose[j+2]));
			}
			timeVector[i] = newPose;
			totalPose.clear();
			newPose.clear();
		}
	}
		...
		num = int(num);
		if (num < 1 || num>timeVector.size()) {
			cerr << "error: fourth parameter must be between 1 and the final frame" << endl;
				return -1;
		}
		swap(timeVector[0], timeVector[num]);
	}
	else if (transform == "left") {
		/*for (unsigned int i = 0; i < timeVector.size(); i++) {
			if (isHigher(timeVector[i])) {
				timeVector.erase(i);
			}
		}*/
		remove_if(timeVector.begin(), timeVector.end(), isHigher);
	}
	else if (transform == "transform") {
		for (unsigned int i = 0; i < timeVector.size(); i++) {
			for (unsigned int j = 0; j < timeVector[i].size(); j++) {
				timeVector[i][j].X(num);
				timeVector[i][j].Y(num);
				timeVector[i][j].Z(num);
			}
		}
	}
	return 0;
...
int stick::posesPrint(string outFile) {
	if (!openOutFile(outFile)) return -1;
	for (unsigned int i = 0; i < timeVector.size(); i++) {
		vector<Point3D> poseVector = timeVector[i];
...
int stick::selfSplit(const stick& inStick, int lastFrame, int firstFrame) {
	for (int j = firstFrame * 25; j < lastFrame * 25; j++) {
		allX.push_back(inStick.allX[j]);
		allY.push_back(inStick.allY[j]);
		allZ.push_back(inStick.allZ[j]);
		/*if (j % 25 == 0) {
			SpineX.push_back(inStick.allX[j]);
			SpineY.push_back(inStick.allY[j]);
			SpineZ.push_back(inStick.allZ[j]);
		}*/
	}
	for (int i = firstFrame; i < lastFrame; i++) {
		SpineX.push_back(inStick.SpineX[i]);
		SpineY.push_back(inStick.SpineY[i]);
		SpineZ.push_back(inStick.SpineZ[i]);
	}
...
int stick::unscaledLegalFile(ifstream& inFile) {
	string lineOf75 = "";
	int lineCount = 0;
...
//	//get to base case
//	if (i > base) {
//		recursiveMatch(distMatrix, i - 1, base);
//	}
//	//create new vector
//	if (i == base) {
...
//		}
//	}
//	//cout << "firstvalue: " << poseDistances[0] << endl;
