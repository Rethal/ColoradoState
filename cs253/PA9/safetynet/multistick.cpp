...
int multistick::selfCompare(const stick& firstIn) {
	...
	for (unsigned int i = 29; i < (endFrame-i); i++) {
		stick Selfiestick(firstIn, i, 0);
		//cout << Selfiestick.timeVector.size() << endl;
		stick LongSelfie(firstIn, endFrame, (i+1));
		//cout << LongSelfie.timeVector.size() << endl;
		comparison = singleDistCompare(Selfiestick, LongSelfie, "L2");
		//cout << minValue << endl;
		//cout << comparison << endl;
		if (comparison == -1) {
			return -1;
		...
		cout << "stop position: " << i << " distance: " << comparison << endl;
	}
	cout << "0" << endl;
	cout << savedArg << endl;
	cout << minValue << endl;
