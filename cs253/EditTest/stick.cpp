...
double stick::recursiveMatch(const vector<vector<double>>& distMatrix, int i, unsigned int end, int base) {
	double minValuePrev;
	cout << i << endl;
	...
	if (i == -1) {
		...
		mappedDistances.push_back(distMatrix[base]);
		i = distMatrix.size()-1;
	}
	//get to base case
	if (i > base) {
		...
		for (unsigned int j = 0; j < lineLength; j++) {
			...
			minValuePrev = recursiveMatch(distMatrix, i - 1, end, base);
	...
	if (i == base) {
		...
		vector<double> poseDistances = distMatrix[i];
		unsigned int lineLength = distMatrix[i].size();
