''' Build a heap out of the elements in A '''
def initHeap(A):
       P = Heap()
       P._H = A[:]
       P._size = len(A)
...
class Heap(object):
   ...
   def child (self, pos, i):
      childIndex = self.childIndex(pos, i)
      if childIndex >= self._size: return None
      else: return self._H[childIndex]
   ...
   def children(self, pos):
      if 0 > pos or pos <= self._size:
          A = [self.child(pos,i) for i in range(0,2)]
   ...
   def heapify(self, pos):
       ...
       if len(C) > 0:
          minChild = min(C)
          if minChild < self._H[pos]:
             childNum = C.index(minChild)
             childPos = self.childIndex(pos, childNum)
             self.swap(pos, childPos)
             self.heapify(childPos)  # recursive call on subtree
   ...
   '''  Shuffle the subtree rooted at pos to get it into heap order '''
   def shuffleHeap (self, pos):
       for i in [0,1]:    #  Shuffle the subtrees rooted at children into heap order ...
           childIndex = self.childIndex(pos, i)
           if childIndex <= self._size:
               self.shuffleHeap(childIndex)
       self.heapify(pos)  #  Now the heap property can only be violated at the
                          #  root, so fix this with a call to heapify
