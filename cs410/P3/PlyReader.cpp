...
int PlyModel::getVertices(ifstream& infile) {
...
int PlyModel::vecAvg(vector<double>& meanFind) {
	double mean;
...
int PlyModel::meanVertexCorrect() {
	vector<double> newX;
	vector<double> newY;
	vector<double> newZ;
	double xMean = meanVertex[0];
...
int PlyModel::whiteVertexCorrect() {
	vector<double> newX;
	vector<double> newY;
	vector<double> newZ;
	double xDev = currentDev[0];
...
int PlyModel::projOneRun(string Title) {
	meanVertexFind();
	stdDeviations();
...
int PlyModel::printToConsole() {
	return 0;
}
int PlyModel::printNorm(string Title) {
...
int PlyModel::printWhite(string Title) {
	string rawname = Title.substr(0, Title.find_last_of("."));
