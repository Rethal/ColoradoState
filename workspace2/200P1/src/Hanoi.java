...
public class Hanoi implements StackIF{
	...
	// null is not an empty array list! Do a proper new
		ArrayList rts = new ArrayList <Object>();
		this.rts = rts;
	}
	...
	private void rtsHanoi() throws StackException{
		...
		while(!isEmpty()){
			...
			if(n==0){}
				...
				check.setState(1);
				Frame next = new Frame(n-1,from,via);
				push(check);
				push(next);
			}
			else if(check.getState()==1){
				System.out.println("move disk " + n + 
						" from " + check.getFrom() + " to " + check.getTo());
				count++;
				check.setState(2);
				Frame next = new Frame(n-1,via,to);
				push(check);
				push(next);
			}
