...
public class BST{
	...
	//				newSubTree = new BSTNode<IdVal>(item);
				...
				if (key.compareTo(retrieveItem(node.getLeft(),key).getKey())==0){
					LeftmostRight = node.getLeft().getRight();
					while(LeftmostRight.getLeft()!=null){
						LeftmostRight = LeftmostRight.getLeft();
					}
					node.setLeft(LeftmostRight);
					node = node.getLeft().getRight();
					while(LeftmostRight.getLeft().getLeft()!=null){
						node = node.getLeft();
					}
					node.setLeft(null);
				}
				...
				if (key.compareTo(retrieveItem(node.getRight(),key).getKey())==0){
					LeftmostRight = node.getRight().getRight();
					while(LeftmostRight.getLeft()!=null){
						LeftmostRight = LeftmostRight.getLeft();
					}
					node.setRight(LeftmostRight);
					node = node.getRight().getRight();
					while(LeftmostRight.getLeft().getLeft()!=null){
						node = node.getLeft();
					}
					node.setLeft(null);
		...
		System.out.println(treeItem+" deleted");
	}
