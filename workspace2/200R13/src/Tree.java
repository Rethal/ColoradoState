...
public class Tree {
	...
	private Hashtable<String,Boolean> sym = new Hashtable<String,Boolean>();
	...
	public Boolean postorderEval(){
        ...
        // IMPLEMENT
	//NEW:
    public Boolean postorderEval(TreeNode node, Hashtable<String,Boolean> symTab){
    		// evaluate left tree
    		...
    		// evaluate operator in node and return Boolean result
    			...
    			if(node.getItem().equals("or")){
    				...
    				return true;
    			else if(node.getItem().equals("false"))
    				return false;
    			else{
    				if(symTab.get(node.getItem()).equals("true")){
    					return true;
    				}
    				else return false;
    			}
